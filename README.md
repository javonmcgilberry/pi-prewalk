...
Prewalk is experimental and uses Pi's public extension APIs. It currently targets Pi **0.85.1**, but it may not work with every Pi version. This implementation has not completed a paid controlled benchmark, so the local numbers above are estimates, not measured quality or savings claims.
