...
function createHarness(
	...
	const baseStream: NonNullable<ProviderConfig["streamSimple"]> = (
		...
		void emit("agent_start", { type: "agent_start" }).then(async () => {
			const contextResults = await emit("context", {
				type: "context",
				messages: streamContext.messages,
			});
			const effectiveSelected =
				selected.provider === planner.provider && selected.id === planner.id
					? currentModel
					: selected;
			...
			if (estimatedTokens > Math.max(0, effectiveSelected.contextWindow - 16_384)) {
				...
				output.push({ type: "start", partial: pressure });
				output.push({ type: "done", reason: "stop", message: pressure });
				output.end();
				return;
			}
			...
			for await (const event of source) {
				...
				if (event.type === "start") {
					await emit("message_start", { type: "message_start", message: event.partial });
	...
	const pi = {
		...
		setModel: vi.fn(async (next: Model<"openai-codex-responses">) => {
		...
		setThinkingLevel: vi.fn((level: ThinkingLevel) => {
			...
			if (previousLevel !== level) {
				void emit("thinking_level_select", {
	...
	const context = {
		...
		set model(value: Model<"openai-codex-responses">) {
			currentModel = value;
		},
		get thinkingLevel() {
...
describe("Prewalk extension harness", () => {
	it("registers Prewalk tools but exposes them only while Prewalk is active", async () => {
		const harness = createHarness({
			activeTools: [PREWALK_TODO_TOOL_NAME, "prewalk_assess", "edit", "write", "bash"],
		});
		prewalkExtension(harness.pi);

		expect([...harness.tools.keys()].sort()).toEqual(["prewalk_assess", PREWALK_TODO_TOOL_NAME]);
		...
		const todoSchema = harness.tools.get(PREWALK_TODO_TOOL_NAME)?.parameters as {
			type: string;
			required?: string[];
			properties?: Record<string, { anyOf?: Array<{ const?: string }>; type?: string }>;
		};
		expect(todoSchema.type).toBe("object");
		expect(todoSchema.required).toEqual(["op"]);
		expect(Object.keys(todoSchema.properties ?? {}).sort()).toEqual([
			"items",
			"list",
			"op",
			"phase",
		...
		expect(todoSchema.properties?.op?.anyOf?.map((entry) => entry.const)).toEqual([
			"init",
			"start",
			"done",
			"rm",
		...
		const assessmentSchema = harness.tools.get("prewalk_assess")?.parameters as {
			type: string;
			required?: string[];
			properties?: Record<string, { anyOf?: Array<{ const?: string }> }>;
		};
		expect(assessmentSchema).toMatchObject({ type: "object", required: ["decision"] });
		expect(assessmentSchema.properties?.decision?.anyOf?.map((entry) => entry.const)).toEqual([
			"continue",
			"bypass",
		]);
		await harness.emit("session_start", { type: "session_start", reason: "startup" });
		...
		expect("setModel" in harness.pi).toBe(true);
	});
	...
	it("fails before planning when the active default tool slate cannot prove an edit", async () => {
		...
		expect(harness.notifications.at(-1)).toBe(
			"Prewalk failed: no active mutation-capable tool can prove the first edit. Enable edit, write, apply_patch, bash, powershell, exec_command, or exec.",
		);
	...
	it("blocks non-read-only tools before assessment execution while permitting inspection", async () => {
		...
		expect(
			await inspected.emit("tool_call", {
				...
				toolCallId: "read-1",
				toolName: "read",
				input: { path: "src/orchestration/coordinator.ts" },
			}),
		).toEqual([undefined]);
		...
		for (const toolName of ["edit", "bash", "todo", "subagent"]) {
			...
			).toEqual([expect.objectContaining({ block: true, terminate: true })]);
			await expect(
	...
	it("rejects delayed todo and assessment execution from run A after replacement B", async () => {
		const todo = createHarness();
		prewalkExtension(todo.pi);
		await todo.emit("session_start", { type: "session_start", reason: "startup" });
		await todo.commands.get("prewalk")?.("run", todo.context);
		await todo.emit("tool_execution_start", {
			type: "tool_execution_start",
			toolCallId: "delayed-todo-a",
			toolName: PREWALK_TODO_TOOL_NAME,
			args: { op: "init" },
		});
		await todo.emit("tool_call", {
			type: "tool_call",
			toolCallId: "delayed-todo-a",
			toolName: PREWALK_TODO_TOOL_NAME,
			args: { op: "init" },
		});
		await todo.commands.get("prewalk")?.("cancel", todo.context);
		await todo.commands.get("prewalk")?.("run", todo.context);
		await expect(
			todo.tools.get(PREWALK_TODO_TOOL_NAME)?.execute("delayed-todo-a", {
				op: "init",
				list: [{ phase: "A", items: ["stale"] }],
			}),
		...
		const currentTodo = await todo.tools.get(PREWALK_TODO_TOOL_NAME)?.execute("todo-b", {
			op: "init",
			list: [{ phase: "B", items: ["current"] }],
		});
		expect(currentTodo?.details).toMatchObject({
			phases: [{ name: "B", tasks: [{ content: "current", status: "in_progress" }] }],
		});

		const assessment = createHarness();
		prewalkExtension(assessment.pi);
		await beginAutomaticAssessment(assessment, "Implement the requested cross-cutting change.");
		await assessment.emit("tool_execution_start", {
			type: "tool_execution_start",
			toolCallId: "delayed-assessment-a",
			toolName: "prewalk_assess",
			args: { decision: "continue" },
		});
		await assessment.emit("tool_call", {
			type: "tool_call",
			toolCallId: "delayed-assessment-a",
			toolName: "prewalk_assess",
			args: { decision: "continue" },
		});
		await assessment.commands.get("prewalk")?.("cancel", assessment.context);
		await assessment.commands.get("prewalk")?.("auto", assessment.context);
		await assessment.emit("input", {
			type: "input",
			text: "Implement the replacement cross-cutting change.",
			source: "interactive",
		});
		await assessment.emit("before_agent_start", {
			type: "before_agent_start",
			prompt: "Implement the replacement cross-cutting change.",
			systemPrompt: "system",
			systemPromptOptions: {},
		});
		await expect(
			assessment.tools.get("prewalk_assess")?.execute("delayed-assessment-a", {
				decision: "continue",
			}),
		).rejects.toThrow("stale");
		await assessment.tools.get("prewalk_assess")?.execute("assessment-b", {
			decision: "bypass",
		});
		await assessment.emit("agent_settled", { type: "agent_settled" });
	...
	it("does not restore an explicitly completed run on reload", async () => {
		const first = createHarness();
		prewalkExtension(first.pi);
		await first.emit("session_start", { type: "session_start", reason: "startup" });
		await first.commands.get("prewalk")?.("run", first.context);
	...
	it("does not restore a successfully session-ended run on reload", async () => {
		const first = createHarness();
		prewalkExtension(first.pi);
		await first.emit("session_start", { type: "session_start", reason: "startup" });
		await first.commands.get("prewalk")?.("run", first.context);
		await first.emit("session_shutdown", { type: "session_shutdown", reason: "quit" });
		expect(first.entries.at(-1)?.data).toMatchObject({ event: "session-ended" });
	...
	it("preserves the ordered replayable trajectory at the executor boundary", async () => {
		...
		const trajectory = [
			...
			assistantWithToolCalls(harness.planner, [{ id: "explore", name: "grep" }], 3),
			{
				role: "toolResult",
				toolCallId: "explore",
				toolName: "grep",
		...
		expect(harness.delegatedOptions.at(-1)?.reasoning).toBe("low");
		const forwarded = harness.delegatedContexts.at(-1)?.messages ?? [];
		expect(
			forwarded.map((message: any) =>
				message.role === "custom"
					? message.customType
		...
		expect(
			forwarded.map((message: any) => message.content?.[0]?.text ?? message.content),
		).toEqual([
			"original task",
			"exploration",
			expect.anything(),
	...
	it("ignores provider drift reported by a disposed run", async () => {
		...
		// stale lifecycle events are correlated by the run identity instead.
		await staleStream?.(harness.planner, { messages: [] }).result();
		expect(harness.entries.at(-1)?.data).toMatchObject({
			event: "plan-injected",
	...
	it("keeps recovery possible when completed analytics finalization fails", async () => {
		const harness = createHarness();
		prewalkExtension(harness.pi);
		await harness.emit("session_start", { type: "session_start", reason: "startup" });
		await harness.commands.get("prewalk")?.("run", harness.context);
	...
	it("does not write session-ended when shutdown analytics finalization fails", async () => {
		const harness = createHarness();
		prewalkExtension(harness.pi);
		await harness.emit("session_start", { type: "session_start", reason: "startup" });
		await harness.commands.get("prewalk")?.("run", harness.context);
	...
	it("keeps native routing independent of provider-config replacement", async () => {
		const harness = createHarness();
		prewalkExtension(harness.pi);
		await reachHandoff(harness);
		...
		expect(harness.context.model?.id).toBe(harness.executor.id);
		expect(harness.entries.at(-1)?.data).not.toMatchObject({
			event: "failed",
	...
	it("restores a cancelled run without validating models or reinstalling a route", async () => {
		const first = createHarness();
		prewalkExtension(first.pi);
		await first.emit("session_start", { type: "session_start", reason: "startup" });
		await first.commands.get("prewalk")?.("run", first.context);
	...
	it("restores the planner route when a live run is cancelled", async () => {
		const harness = createHarness();
		prewalkExtension(harness.pi);
		await harness.emit("session_start", { type: "session_start", reason: "startup" });
		await harness.commands.get("prewalk")?.("run", harness.context);
	...
	it("blocks session-tree navigation while a Prewalk run owns the session", async () => {
		const harness = createHarness();
		prewalkExtension(harness.pi);
		await harness.emit("session_start", { type: "session_start", reason: "startup" });
		await harness.commands.get("prewalk")?.("run", harness.context);
		...
		expect(harness.notifications.at(-1)).toContain("blocks session-tree navigation");
	...
	it("replays the planning checkpoint after compaction interrupts the initial planner request", async () => {
		...
		expect(harness.delegated).toEqual([]);
		expect(pressureMessage).toMatchObject({
			provider: harness.planner.provider,
			model: harness.planner.id,
			stopReason: "stop",
			content: [],
		});
		const beforeResume = harness.messages.length;
		await harness.emit("turn_end", {
			type: "turn_end",
			turnIndex: 1,
		...
		await harness.emit("session_compact", {
			type: "session_compact",
			compactionEntry: { type: "compaction", id: "planner-pressure" },
			reason: "manual",
			willRetry: false,
		});
	...
	it("fails closed when planner pressure remains after one compaction retry", async () => {
		...
		await harness.emit("session_compact", {
			type: "session_compact",
			compactionEntry: { type: "compaction", id: "planner-pressure-retry" },
			reason: "manual",
			willRetry: false,
		});
		harness.completeCompaction();
		...
		await harness.emit("session_compact", {
			type: "session_compact",
			compactionEntry: { type: "compaction", id: "native-planner-pressure-retry" },
			fromExtension: false,
			reason: "threshold",
			willRetry: false,
		});
	...
	it("fails closed before planner transport when automatic compaction is disabled", async () => {
		await writeFile(
		...
		expect(harness.notifications).toContain(
			"Prewalk stopped before an oversized planner request because Pi automatic compaction is disabled.",
		);
		expect(harness.notifications.at(-1)).toBe("Prewalk failed: planner-compaction-failed.");
	});
	...
	it("replays the planning checkpoint after Pi compacts the initial planner request", async () => {
		...
		await harness.emit("session_compact", {
			type: "session_compact",
			compactionEntry: { type: "compaction", id: "native-planner-pressure" },
			fromExtension: false,
			reason: "threshold",
			willRetry: false,
		});
